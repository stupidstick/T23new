...
public class TwoThreeTree<K extends Comparable<K>, V> implements Tree<K, V> {
    ...
    public TwoThreeTree(Class<K> keyClass) {
        size = 0;
        root = null;
    ...
    public TwoThreeTree(TwoThreeTree<K, V> tree) {
        this(tree.keyClass);
    ...
    public boolean set(K key, V val) {
        LeafNode searched = searchNode(key, root);
        if (searched == null) {
            return false;
        }
        searched.val = val;
    ...
    private LeafNode searchNode(K key, Node n) {
        LeafNode found = null;
        TreeNode t = null;
        LeafNode l = null;
        if (n instanceof TreeNode) {
        ...
        if (t != null) {
            if (t.degree == 1) {
                found = searchNode(key, t.children[0]);
            } else if (t.degree == 2 && key.compareTo(t.keys[0]) < 0) {
                found = searchNode(key, t.children[0]);
            } else if (t.degree == 2 && key.compareTo(t.keys[0]) >= 0) {
                found = searchNode(key, t.children[1]);
            } else if (t.degree == 3 && key.compareTo(t.keys[0]) < 0) {
                found = searchNode(key, t.children[0]);
            } else if (t.degree == 3 && key.compareTo(t.keys[0]) >= 0 && key.compareTo(t.keys[1]) < 0) {
                found = searchNode(key, t.children[1]);
            } else if (t.degree == 3 && key.compareTo(t.keys[1]) >= 0) {
                found = searchNode(key, t.children[2]);
            }
        } else if (l != null && key.compareTo(l.key) == 0) {
            return l;
        }
