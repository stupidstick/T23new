...
public class BstPane<K extends Comparable<K>, V> extends Pane {
    ...
    public BstPane(TwoThreeTree<K, V> tree) {
        this.tree = tree;
    ...
    protected void displayTree(TwoThreeTree<K, V>.Node node, double x, double y, double hGap, Color color) {
        if (node instanceof TwoThreeTree<K, V>.TreeNode treeNode) {
            if (treeNode.children[0] != null) {
                getChildren().add(new Line(x - hGap, y + vGap, x, y));
                displayTree(treeNode.children[0], x - hGap, y + vGap, hGap / 4, color);
            }

            if (treeNode.children[1] != null) {
                getChildren().add(new Line(x, y + vGap, x, y));
                displayTree(treeNode.children[1], x, y + vGap, hGap / 4, color);
            }
